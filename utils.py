...
def load_model(model_name: str) -> ChatOpenAI:
    ...
    # llm = ChatOpenAI(api_key=SOLAR_API_KEY, model_name=model_name)

    # 실행하면 .env file 내의 환경변수 컴퓨터에 저장
    chat = ChatUpstage(api_key=SOLAR_API_KEY, model=model_name)
    return chat 
...
def initialize_chain(llm: ChatOpenAI, character_name: str, memory: ConversationBufferMemory) -> LLMChain:
    ...
    custom_prompt = ChatPromptTemplate(
        messages=[
            ...
            MessagesPlaceholder(variable_name="chat_history"), # 대화가 저장됨
            HumanMessagePromptTemplate.from_template("{input}"), # input기반으로 메세지 프롬프트 만듦
        ]
    )
